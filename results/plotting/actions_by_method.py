...
def main(args: argparse.Namespace) -> None:
    ...
    for env in plot_envs:
        ...
        env_name = env.capitalize() if env == 'train' else TRANSLATIONS[SEQUENCES[sequence][env]]

        for j, method in enumerate(methods):
            data = get_action_data(folder, iterations, method, n_actions, seeds, sequence)
            title = TRANSLATIONS[method] if env == 'Train' else f'Evaluating on {env_name}'
            ...
            sub_plot.set_title(title, fontsize=16)
        ...
        bottom_adjust = 0.07 if n_envs == 4 else 0.13 if n_methods > 1 else 0.22
        ...
        plt.savefig(f'{file_path}/{sequence}_{env_name}.png')
        plt.savefig(f'{pdf_file_path}/{sequence}_{env_name}.pdf')
        plt.show()
